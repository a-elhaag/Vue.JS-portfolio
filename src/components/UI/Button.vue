...
.icon :deep(svg) { width: 1.6em; height: 1.6em; display: block; fill: currentColor; }
.label { line-height: 1; }

/* Sizes */
.s-sm { padding: 8px 12px; font-size: 13px; }
.s-md { padding: 12px 20px; font-size: 14px; }
.s-lg { padding: 14px 24px; font-size: 16px; }
...
.v-ghost:hover:not(.is-disabled) {
  ...
  color: var(--bg);
  box-shadow:
    0 0 18px 4px color-mix(in oklab, var(--glow) 45%, transparent);
}

/* LINK: no chrome */
.v-link {
  background: transparent; border-color: transparent; color: var(--text); padding: 0;
}
.v-link:hover:not(.is-disabled) { text-decoration: underline; box-shadow: none; transform: none; }
...
.v-social .label { display: none; } /* icon-only */
